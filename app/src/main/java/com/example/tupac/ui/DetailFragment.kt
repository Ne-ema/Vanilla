...
import coil.load
import com.example.tupac.adapter.CommentAdapter
import com.example.tupac.databinding.FragmentDetailBinding
...
class DetailFragment : Fragment() {
    ...
    private fun setRecyclerView() {
        ...
        binding.CommentRecyclerViewer.adapter = ScaleInAnimationAdapter(alphaAdapter).apply {
            setDuration(1000)
            setHasStableIds(false)
            setFirstOnly(false)
            setInterpolator(OvershootInterpolator(.100f))
        }
        binding.CommentRecyclerViewer.layoutManager = LinearLayoutManager(requireContext())
    }
